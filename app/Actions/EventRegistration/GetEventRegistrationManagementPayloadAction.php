...
{
    /**
     * @return array{metrics: array{eventsOpen: int, totalRegistrations: int, confirmedRegistrations: int, pendingRegistrations: int}, events: array<int, array{id: int, name: string, slug: string, organizationName: string, isOpen: bool, capacity: int|null, registeredCount: int, remainingQuota: int|null, requirePayment: bool, publicUrl: string}>, registrations: array<int, array{id: int, participantName: string, participantEmail: string, phone: string, institution: string, status: string, registeredAt: string, projectName: string}>}
     */
    public function execute(int $actorUserId): array
    {
        ...
        $events = DB::table('event_registration_settings')
            ->join('projects', 'projects.id', '=', 'event_registration_settings.project_id')
            ...
            ->orderByDesc('event_registration_settings.is_open')
